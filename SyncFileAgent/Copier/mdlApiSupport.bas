...
Public sCopyFileSource As String, sCopyFileDest As String, lCancelCopyFile As Long
...
Public Function CopyFile(ByVal Source As String, ByVal Dest As String, Optional Results As String = "") As Boolean

    On Error GoTo CopyFile_EH

    Dim lRet As Long
    Dim lErrNumber As Long, sRet As String, bRet As Boolean
    
100     sCopyFileSource = Source
102     sCopyFileDest = Dest
    
        'Clear the last DLL error number
104     SetLastError 0&

106     lRet = CopyFileEx(Source, Dest, AddressOf CopyProgressRoutine, ByVal 0&, lCancelCopyFile, COPY_FILE_RESTARTABLE)

108     If lRet = 0 Then
110         lErrNumber = Err.LastDllError
112         sRet = "Error [" & lErrNumber & "] " & GetLastApiError(lErrNumber) & " occurred in ApiSupport.CopyFile()"
            'Debug.Print sRet
114         bRet = False
...
126     sRet = "Error [" & Err.Number & "] " & Err.Description & " occurred in ApiSupport.CopyFile()" & IIf(Erl <> 0, " at line " & Erl, "")
    
        On Error Resume Next

128     bRet = False
    
130     Resume CopyFile_End
...
Dim bCancel As Boolean
    ...
    frmMain.UpdateStatus "Copying " & GetFileName(sCopyFileSource) & " ... " & CStr(Int((TotalBytesTransferred * 10000) / (TotalFileSize * 10000) * 100)) & "% complete", False, bCancel

    CopyProgressRoutine = IIf(bCancel, PROGRESS_CANCEL, PROGRESS_CONTINUE)
...
128     sErrMsg = "Error [" & Err.Number & "] " & Err.Description & " occurred in ApiSupport.GetINISection()" & IIf(Erl <> 0, " at line " & Erl, "")
...
112     sErrMsg = "Error [" & Err.Number & "] " & Err.Description & " occurred in ApiSupport.GetINISetting()" & IIf(Erl <> 0, " at line " & Erl, "")
...
104     sErrMsg = "Error [" & Err.Number & "] " & Err.Description & " occurred in ApiSupport.SaveINISetting()" & IIf(Erl <> 0, " at line " & Erl, "")
